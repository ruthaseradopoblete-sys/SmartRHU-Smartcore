...
const MS: Record<string, React.CSSProperties> = {
  overlay: { position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', justifyContent:'center', alignItems:'flex-start', zIndex:1000, padding:'20px', overflowY:'auto' },
  modal: { background:'white', width:'100%', maxWidth:'820px', borderRadius:'8px', position:'relative', fontFamily:'Arial, sans-serif', marginTop:'10px', marginBottom:'10px' },
  header: { background:'#1a6b2e', color:'white', textAlign:'center', padding:'10px 40px', fontSize:'15px', fontWeight:'bold', borderRadius:'6px 6px 0 0' },
  body: { padding:'16px 24px', background:'#f5f7f5' },
  closeBtn: { position:'absolute', top:'6px', right:'14px', background:'none', border:'none', fontSize:'28px', color:'#8B1A1A', cursor:'pointer', fontWeight:'bold', lineHeight:1, zIndex:10 },
...
function AddPatientModal({ isOpen, onClose, onSaved }: { isOpen:boolean; onClose:()=>void; onSaved:()=>void }) {
  ...
  const doSave = async () => {
    ...
    try {
      ...
      // ── 14. SOAP CONSULTATION ──────────────────────────────────────────────
      // This is the KEY insert that sends the patient to the doctor's Today Queue.
      // queue_date = today ensures PendingPatients filters it correctly.
      // The DB trigger assign_soap_queue_number auto-assigns the queue number.
      ...
      if (soapError) console.warn('soap_consultations skipped:', soapError.message)
    ...
    <div style={MS.overlay}>
      <div style={MS.modal}>
              ...
              <div style={{ flex:1, display:'flex', gap:'6px', flexWrap:'wrap' }}>
                {(['lastName','firstName','middleName'] as const).map((k,i) => (
                  <div key={k} style={{ display:'flex', flexDirection:'column', flex:1 }}>
                    ...
                    <span style={{ fontSize:'10px', color:'#555', textAlign:'center' }}>{['Last','First','Middle'][i]}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={MS.row}>
              ...
              <div style={{ flex:1, display:'flex', gap:'6px', flexWrap:'wrap' }}>
                {(['purok','barangay','municipality'] as const).map((k,i) => (
                  <div key={k} style={{ display:'flex', flexDirection:'column', flex:1 }}>
                    <input style={MS.inputFlex} value={s1[k]} onChange={e => setS1(p => ({ ...p, [k]: e.target.value }))} />
                    <span style={{ fontSize:'10px', color:'#555', textAlign:'center' }}>{['Purok','Barangay','Municipality'][i]}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {step === 2 && (<>
          ...
          <div style={MS.body}>
            <div style={{ display:'flex', gap:'24px' }}>
              {[{title:'Past Medical History', med:pastMed, setMed:setPastMed, spec:pastMedSpec, setSpec:setPastMedSpec},
                {title:'Family History',        med:famHist, setMed:setFamHist, spec:famHistSpec, setSpec:setFamHistSpec}]
                .map(({ title, med, setMed, spec, setSpec }) => (
                <div key={title} style={{ flex:1 }}>
                  <div style={{ fontWeight:'bold', fontSize:'14px', marginBottom:'8px' }}>{title}</div>
                  {DISEASES.map(d => (
                    <div key={d} style={{ marginBottom:'3px' }}>
                      <label style={{ display:'flex', alignItems:'center', gap:'5px', fontSize:'13px', cursor:'pointer' }}>
                        ...
                        {d}
                        {(d==='Allergy'||d==='Cancer'||d==='Hepatitis') && (<> (Specify: <IInput width="70px" value={spec[d]||''} onChange={e => setSpec(p => ({ ...p, [d]: e.target.value }))} />)</>)}
                        {d==='Hypertension' && (<> (Highest BP: <IInput width="80px" value={spec[d]||''} onChange={e => setSpec(p => ({ ...p, [d]: e.target.value }))} /> mmHg)</>)}
                        {d==='PTB' && (<> (Specify Extra: <IInput width="70px" value={spec[d]||''} onChange={e => setSpec(p => ({ ...p, [d]: e.target.value }))} />)</>)}
                      </label>
                    </div>
                  ))}
              ...
              <div style={{ flex:1 }}>
                ...
                {([['Smoking',smoking,setSmoking],['Alcohol',alcohol,setAlcohol],['Illicit Drugs',illicit,setIllicit],['Sexuality Active',sexually,setSexually]] as [string,string,React.Dispatch<React.SetStateAction<string>>][]).map(([label,val,setVal]) => (
                  <div key={label} style={{ marginBottom:'8px' }}>
                    ...
                    {label==='Smoking' && <div style={{ ...MS.row, marginLeft:'110px' }}><span style={{ fontSize:'13px' }}>No. of Packs-Year:</span><IInput width="100px" value={packsYear} onChange={e => setPacksYear(e.target.value)} /></div>}
                    {label==='Alcohol' && <div style={{ ...MS.row, marginLeft:'110px' }}><span style={{ fontSize:'13px' }}>No. of Servings/day:</span><IInput width="100px" value={servingsDay} onChange={e => setServingsDay(e.target.value)} /></div>}
                  </div>
                ))}
              </div>
              <div style={{ flex:1 }}>
              ...
              {([
                ...
                <div key={title}>
                  ...
                  <div style={{ display:'flex', gap:'12px' }}>
                    <div style={{ flex:1 }}>{items.slice(0,Math.ceil(items.length/2)).map(item => <CB key={item} label={item} checked={!!state[item]} onChange={() => togCB(state, setState, item)} />)}</div>
                    <div style={{ flex:1 }}>
                      {items.slice(Math.ceil(items.length/2)).map(item => <CB key={item} label={item} checked={!!state[item]} onChange={() => togCB(state, setState, item)} />)}
                      <label style={{ display:'flex', alignItems:'center', gap:'4px', fontSize:'13px', marginTop:'2px', cursor:'pointer' }}>
                        ...
                        Others: <input style={{ ...MS.input, width:'80px' }} value={other} onChange={e => setOther(e.target.value)} />
                      </label>
                    </div>
                  </div>
                </div>
        ...
        {step === 8 && (<>
          ...
          <button style={MS.closeBtn} onClick={() => setConfirm('close')}>×</button>
          <div style={MS.body}>
            {([['q1','1. Eats processed food (ex. Instant Noodles, Burgers, Fries, Fried Chicken, etc) and ihaw-ihaw weekly?'],['q2','2. Eats 3 serving of fruits and vegetable Daily?'],['q3','3. Does at least 2.5 hours of moderate walking-intensity physical activity every week?']] as [string,string][]).map(([key,q]) => (
            ...
            <div style={{ marginBottom:'12px' }}>
              ...
              <div style={{ display:'flex', justifyContent:'center', gap:'40px' }}>
                {['Polyphagia','Polydipsia','Polyuria'].map(s => <CB key={s} label={s} checked={!!symptoms[s]} onChange={() => togCB(symptoms, setSymptoms, s)} />)}
              </div>
            </div>
        ...
        {step === 9 && (<>
          ...
          <div style={MS.body}>
            <div style={{ ...MS.row, marginBottom:'12px' }}>
              <span style={{ fontWeight:'bold', fontSize:'14px' }}>Angina or Heart Attack</span>
            ...
            {([
              ...
              { k:'a5', q:'5. Does the pain go away if you stand still or get medication?', t:'(Nawawala ba yung sakit sa dibdib kapag ikaw ay tumitigil o umiinom ng gamot sa ilalim ng dila?)' },
          ...
          <div style={MS.body}>
            ...
            <div style={{ marginBottom:'14px' }}>
              <p style={{ fontSize:'14px', marginBottom:'6px' }}>
                ...
                <em style={{ fontStyle:'italic', fontSize:'13px' }}>(Nakaramdam ka na ba ng pagkautal, panghihina ng braso o binti, o pamamanhid ng kalahati ng katawan?)</em>
              </p>
              ...
              <p style={{ fontSize:'12px', fontStyle:'italic', marginLeft:'20px', color:'#333' }}>• If YES to number 8, you may have TIA/Stroke, must seek a doctor.</p>
            </div>
            <div style={{ marginBottom:'14px' }}>
