...
// ── Step 1: validate, then open review/confirmation ────
  ...
  async function executeSend() {
    ...
    try {
      ...
      // Core columns that always exist in the prescriptions table.
      const baseRows = selectedMeds.map(med => ({
      ...
      // Optional tagging columns. These only exist if the prescriptions table
      // has been migrated to include them. If the table lacks them, we retry
      // the insert without them so the save never fails on a schema mismatch.
      const taggingCols = {
      ...
      const fullRows = baseRows.map(r => ({ ...r, ...taggingCols }));

      // First attempt: full insert with tagging columns.
      let prescError = (await supabase.from("prescriptions").insert(fullRows)).error;

      // Fallback: if the table is missing any of the optional tagging columns,
      // Postgres / PostgREST returns a "Could not find the '<col>' column …
      // in the schema cache" error. In that case, retry with core columns only.
      const isMissingColumn =
        prescError &&
        (/schema cache/i.test(prescError.message) ||
          /could not find/i.test(prescError.message) ||
          (prescError as any).code === "PGRST204");

      if (isMissingColumn) {
        console.warn(
          "prescriptions table is missing nurse-tagging columns; saving without them.",
          prescError?.message
        );
        prescError = (await supabase.from("prescriptions").insert(baseRows)).error;
      }
      ...
      await logAction({
        user_name: user?.name || currentUserName || "",
