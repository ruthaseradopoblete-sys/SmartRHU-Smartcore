...
import { QueueEntry } from "./PatientQueue";
...
interface Props {
  ...
  onClose: () => void;
  onSave: () => void;
  onOpenPresc?: (entry?: QueueEntry) => void;
  onOpenLab?: (entry?: QueueEntry) => void;
}
        ...
        >
            ...
            <div>
              ...
              >
                <VitalBadge
                  ...
                  value={d.physical.blood_pressure_mmhg}
                  unit="mmHg"
                />
                <VitalBadge
                  icon="💗"
                  label="Heart Rate"
                  value={d.physical.heart_rate_bpm}
                  unit="bpm"
                />
                <VitalBadge
                  icon="🌡️"
                  label="Temperature"
                  value={d.physical.temperature_c}
                  unit="°C"
                />
                <VitalBadge
                  icon="🫁"
                  label="Resp. Rate"
                  value={d.physical.respiratory_rate_cpm}
                  unit="cpm"
                />
                <VitalBadge
                  icon="⚖️"
                  label="Weight"
                  value={d.physical.weight_kg}
                  unit="kg"
                />
                <VitalBadge
                  icon="📏"
                  label="Height"
                  value={d.physical.height_cm}
                  unit="cm"
                />
              </div>
              {(d.physical.blood_type ||
                d.physical.visual_acuity_right_eye) && (
                <div
                  style={{
                    marginTop: 8,
                    display: "flex",
                ...
                >
                  {d.physical.blood_type && (
                    <InfoItem
                      label="Blood Type"
                      value={d.physical.blood_type}
                    />
                  )}
                  {d.physical.visual_acuity_right_eye && (
                    <InfoItem
                      label="Visual Acuity R"
                      value={d.physical.visual_acuity_right_eye}
                    />
                  )}
                  {d.physical.visual_acuity_left_eye && (
                    <InfoItem
                      label="Visual Acuity L"
                      value={d.physical.visual_acuity_left_eye}
                    />
                  )}
                </div>
              )}
          ...
          {conditions.length > 0 && (
            <div>
              ...
              >
                {d.pastMed?.cancer_specify && (
                  <InfoItem
                    label="Cancer type"
                    value={d.pastMed.cancer_specify}
                  />
                )}
                {d.pastMed?.hypertension_highest_bp && (
                  <InfoItem
                    label="Highest BP"
                    value={d.pastMed.hypertension_highest_bp}
                  />
                )}
                {d.pastMed?.past_surgeries_done && (
                  <InfoItem
                    label="Past surgeries"
                    value={d.pastMed.past_surgeries_done}
                  />
                )}
              </div>
            </div>
            ...
            hasAny({
              ...
              <div>
                ...
                <InfoGrid>
                  <InfoItem
                    label="LMP"
                    value={d.menstrual.last_menstrual_period}
                  />
                  <InfoItem
                    label="Menarche Age"
                    value={d.menstrual.menarche_age}
                  />
                  <InfoItem
                    label="Cycle (days)"
                    value={d.menstrual.interval_cycle_days}
                  />
                  <InfoItem
                    label="Duration (days)"
                    value={d.menstrual.period_duration_days}
                  />
                  <InfoItem
                    label="Pads / Day"
                    value={d.menstrual.pads_per_day}
                  />
                  <InfoItem
                    label="Menopause"
                    value={d.menstrual.menopause ? "Yes" : null}
                  />
                </InfoGrid>
              </div>
            )}
          ...
          {isFemale &&
            d.pregnancy &&
            hasAny({ g: d.pregnancy.gravida, p: d.pregnancy.para }) && (
              <div>
                <div
                  style={{
                    fontSize: 10,
                  ...
                  {(
                    ...
                    d.pregnancy[k] != null ? (
                      ...
                      >
                        ...
                        <div
                          style={{ fontSize: 20, fontWeight: 800, color: DARK }}
                        >
                          {d.pregnancy[k]}
                        </div>
                      </div>
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#4b6557" }}>
              Forwarded to registrar queue automatically
            </div>
          </div>
        </div>
        <div
        ...
        >
          <div style={{ display: "flex", gap: 8, fontSize: 13 }}>
            <span style={{ color: "#6b7280", minWidth: 60, fontSize: 11 }}>
              Date
            </span>
            <span style={{ fontWeight: 700, color: DARK }}>
              {new Date(followUp.date + "T00:00:00").toLocaleDateString(
                "en-PH",
                {
                  year: "numeric",
                  month: "long",
      ...
      >
        <div style={{ flex: 1, minWidth: 150 }}>
          ...
          <input
            ...
            value={followUp.date}
            onChange={(e) =>
              setFollowUp((f) => ({ ...f, date: e.target.value, error: "" }))
            }
            style={{
              width: "100%",
...
// ══ MAIN MODAL ════════════════════════════════════════════
export default function NurseSoapModal({
  open,
  entry,
  onClose,
  onSave,
  ...
  async function loadAll(consultId: string, patientId: string) {
    try {
      const [cR, phR, pmR, fhR, shR, mnR, prR, imR, fuR, vrR] =
        await Promise.all([
          supabase
      ...
      setIsDone(done);
      setConsultDate(
        consultData?.consultation_date ?? consultData?.queue_date ?? "",
      );

      const assessmentText =
        Array.isArray(consultData?.assessments) &&
        consultData.assessments.length > 0
  ...
  function preparePatientForNextModal(
    target: "prescription" | "lab" | "vaccine",
  ) {
    if (!entry) return;

    const payload = {
      source: "nurse_consultation",
      target,
      queueId: entry.queueId,
      consultationId: consultationId || entry.queueId,
    ...
    try {
      ...
      sessionStorage.setItem(
        "selectedConsultationPatient",
        JSON.stringify(payload),
      );
      window.dispatchEvent(
        new CustomEvent("consultationPatientSelected", { detail: payload }),
      );
    } catch (e) {
      console.warn("Unable to cache consultation patient:", e);
    }
  }
  ...
  async function handleSave() {
    ...
    if (error) {
      console.error("Nurse SOAP save error:", error);
    ...
    try {
      await logAction({
        ...
        module: "Consultation",
        description: `Saved nurse SOAP notes for patient: ${entry.name}`,
        status: "success",
      });
    } catch (e) {
  ...
  async function handleSaveFollowUp() {
    ...
    const { error } = await supabase.from("follow_up_schedules").upsert(
      {
        patient_id: entry.patientId,
        consultation_id: consultationId || entry.queueId,
    ...
    if (!error) {
      await supabase
        ...
        .eq("id", consultationId || entry.queueId);
    }
      ...
      {showPostSave && (
        ...
        >
          ...
          >
            ...
            >
              SOAP notes for <strong>{entry.name}</strong> have been recorded by
              the nurse.
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <button
                onClick={() => {
                  preparePatientForNextModal("prescription");
                  setShowPostSave(false);
                  onSave();
                  onOpenPresc?.(entry);
                }}
              ...
              <button
                onClick={() => {
                  preparePatientForNextModal("lab");
                  setShowPostSave(false);
                  onSave();
                  onOpenLab?.(entry);
                }}
              ...
              {!vaccineSent ? (
                <button
                  onClick={() => {
                    preparePatientForNextModal("vaccine");
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {SOAP_CFG.map(
                  ({ key, label, icon, ph, bg, border, color, tag }) => (
                    <div
                      key={key}
                      style={{
                        background: bg,
                        ...
                        overflow: "hidden",
                        opacity:
                          isDone && !soap[key as keyof typeof soap] ? 0.5 : 1,
                      }}
                    >
                      <div
                        style={{
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          !isDone &&
                          setSoap((f) => ({ ...f, [key]: e.target.value }))
                        }
                        readOnly={isDone}
