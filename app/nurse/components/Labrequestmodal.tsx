...
type QueuePatient = {
  id: string;         // nurse_consultation_queue.id
  patient_id: string; // actual patient UUID
  name: string;
  age: string;
  gender: string;
  status: "pending" | "done";
...
const inputStyle: React.CSSProperties = {
  ...
  background: "#f0faf4", outline: "none", boxSizing: "border-box",
};
        ...
        <div style={{ overflowY: "auto", padding: "20px 24px", flex: 1 }}>
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {[{ label: "Age", val: age || "—" }, { label: "Gender", val: gender || "—" }, { label: "Civil Status", val: civilStatus || "—" }].map(({ label, val }) => (
                <div key={label}>
          ...
          {Object.keys(groupedTests).length === 0 && !ultrasound && !xray && !others ? (
            <div style={{ padding: "16px", background: "#fef9c3", borderRadius: 8, fontSize: 13, color: "#92400e", textAlign: "center" }}>
              No tests selected.
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {Object.entries(groupedTests).map(([section, labels]) => (
                <div key={section} style={{ border: "1px solid #d1fae5", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ background: "#dcfce7", padding: "6px 14px", fontSize: 11, fontWeight: 700, color: "#166534", letterSpacing: 0.5 }}>
                    {section.toUpperCase()}
                  </div>
              ...
              {(ultrasound || xray || others) && (
                <div style={{ border: "1px solid #d1fae5", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ background: "#dcfce7", padding: "6px 14px", fontSize: 11, fontWeight: 700, color: "#166534", letterSpacing: 0.5 }}>OTHERS</div>
                  <div style={{ padding: "8px 14px", display: "flex", flexDirection: "column", gap: 4 }}>
                    {ultrasound && <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1f2937" }}><span style={{ color: "#16a34a", fontSize: 16 }}>✓</span>Ultrasound: <strong>{ultrasound}</strong></div>}
                    {xray      && <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1f2937" }}><span style={{ color: "#16a34a", fontSize: 16 }}>✓</span>X-ray: <strong>{xray}</strong></div>}
                    {others    && <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1f2937" }}><span style={{ color: "#16a34a", fontSize: 16 }}>✓</span>Others: <strong>{others}</strong></div>}
                  </div>
                </div>
              )}
            </div>
          ...
          <div style={{ marginTop: 16, padding: "10px 14px", background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 8, fontSize: 12, color: "#92400e", display: "flex", alignItems: "center", gap: 8 }}>
            ✓ After confirming, the lab request will be sent to the laboratory.
          </div>
        </div>

        <div style={{ padding: "14px 24px", borderTop: "1px solid #e5e7eb", display: "flex", justifyContent: "flex-end", gap: 10, background: "#fafafa", borderRadius: "0 0 16px 16px" }}>
          <button onClick={onBack} disabled={saving} style={{ padding: "9px 22px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", fontSize: 14, cursor: saving ? "not-allowed" : "pointer", fontWeight: 600, color: "#374151", display: "flex", alignItems: "center", gap: 6 }}>← Back</button>
          <button onClick={onConfirm} disabled={saving} style={{ padding: "9px 28px", borderRadius: 8, border: "none", background: saving ? "#6b7280" : "#1a6b3a", color: "#fff", fontSize: 14, cursor: saving ? "not-allowed" : "pointer", fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>
...
// ─── Main Modal ───────────────────────────────────────────────────────────────
export default function NurseLabRequestModal({ open, patient, onClose, onSent, onSend }: Props) {
  ...
  const [date,              setDate]               = useState(getTodayPHDate());
  ...
  useEffect(() => {
    if (!open) return;
    setChecked({}); setUltrasound(""); setXray(""); setOthers("");
    setError(null); setFetchError(null);
    setShowPreview(false); setShowCloseConfirm(false);
    setDate(getTodayPHDate());
    setLoadingPatients(true);
    fetchTodayPatients();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, patient]);

  // ── Fetch ALL of today's nurse_consultation_queue (pending + done) ─────────
  async function fetchTodayPatients() {
    ...
    const rows: QueuePatient[] = (data ?? []).map((r: any) => ({
      id:         r.id,
    ...
    setQueuePatients(rows);

    if (patient?.id) {
      const match = rows.find(r => r.patient_id === patient.id);
      if (match) {
  ...
  function handlePatientSelect(queueRowId: string) {
    ...
    if (queueRowId) {
      ...
      setSelectedPatientId(""); setManualName(""); setManualAge("");
      setManualGender(""); setManualCivilStatus(""); setManualAddress("");
    }
  }
  ...
  function hasUnsavedChanges() {
    return hasAnyTest || (!patient && !selectedQueueId && (manualName.trim() !== "" || manualAddress.trim() !== ""));
  }
  ...
  function handleReview() {
    if (!selectedPatientId && !manualName.trim()) { setError("Please select or enter a patient name."); return; }
    if (!hasAnyTest) { setError("Please select at least one test."); return; }
    setError(null);
    setShowPreview(true);
  }
  ...
  async function handleConfirmedSend() {
    ...
    if (dbError) { setShowPreview(false); setError(dbError.message); return; }

    await logAction({
      user_role: "Nurse", user_name: "Nurse",
      action: "SEND_LAB_REQUEST", module: "Laboratory Request",
  ...
  const SectionHeader = ({ title }: { title: string }) => (
    <div style={{ background: "#1a6b3a", color: "#fff", padding: "7px 14px", borderRadius: 6, fontSize: 12, fontWeight: 700, letterSpacing: 1, marginBottom: 10 }}>
      {title}
    </div>
  );
  ...
  const selectedCount  = Object.values(checked).filter(Boolean).length + (ultrasound ? 1 : 0) + (xray ? 1 : 0) + (others ? 1 : 0);

  return (
    <>
      <PreviewModal
        open={showPreview} name={manualName} age={manualAge} gender={manualGender}
        civilStatus={manualCivilStatus} address={manualAddress} date={date}
        checked={checked} ultrasound={ultrasound} xray={xray} others={others}
        onBack={() => setShowPreview(false)} onConfirm={handleConfirmedSend} saving={saving}
      />

      <div style={{ position: "fixed", inset: 0, width: "100vw", height: "100vh", background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16, boxSizing: "border-box", overflow: "hidden" }}>
        <div style={{ background: "#fff", borderRadius: 16, width: "min(980px, calc(100vw - 32px))", maxWidth: "100%", height: "min(92vh, 820px)", maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 8px 32px rgba(0,0,0,0.25)", overflow: "hidden" }}>

          {/* Header */}
          <div style={{ background: "#1a6b3a", padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
            <div>
              <span style={{ color: "#fff", fontWeight: 700, fontSize: 18, display: "flex", alignItems: "center", gap: 8 }}>
                🧪 Laboratory Request
              </span>
              {/* ← CHANGED from "Doctor → Laboratory test order" */}
              <div style={{ color: "#a7f3c2", fontSize: 12, marginTop: 2 }}>
                Nurse → Laboratory test order
              </div>
            </div>
            <button onClick={requestClose} style={{ background: "rgba(255,255,255,0.2)", border: "none", color: "#fff", width: 32, height: 32, borderRadius: "50%", cursor: "pointer", fontSize: 18 }}>×</button>
          </div>

          <div style={{ display: "flex", flex: 1, minHeight: 0 }}>

            {/* LEFT PANE */}
            <div style={{ width: 320, flexShrink: 0, borderRight: "1px solid #e5e7eb", overflowY: "auto", padding: "18px 20px", background: "#fcfdfc" }}>
              ...
              <div style={{ fontSize: 11, fontWeight: 700, color: "#1a6b3a", letterSpacing: 1, marginBottom: 8, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
                SELECT FROM TODAY'S QUEUE
                {queuePatients.length > 0 && (
                  <span style={{ background: "#dcfce7", color: "#166534", borderRadius: 99, padding: "1px 8px", fontSize: 10, fontWeight: 700 }}>
              ...
              {/* ── Dropdown: pending (⏳ Waiting) then done (✓ Consulted) ── */}
              <select
                value={selectedQueueId}
                onChange={e => handlePatientSelect(e.target.value)}
                disabled={loadingPatients}
                style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid #d1d5db", fontSize: 14, marginBottom: 10, background: "#f0faf4", color: selectedQueueId ? "#111" : "#6b7280", cursor: loadingPatients ? "wait" : "default", boxSizing: "border-box" }}
              >
                <option value="">
                  {loadingPatients
                    ? "Loading patients…"
                ...
                {pendingCount > 0 && (
                  <optgroup label="⏳ Waiting">
                    {queuePatients.filter(p => p.status === "pending").map(p => (
                ...
                {doneCount > 0 && (
                  <optgroup label="✓ Consulted">
                    {queuePatients.filter(p => p.status === "done").map(p => (
                      <option key={p.id} value={p.id}>
                        {p.name}{p.age ? ` · ${p.age} yrs` : ""}{p.gender ? ` · ${p.gender}` : ""}
                      </option>
                    ))}
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input value={manualName} onChange={e => { setManualName(e.target.value); if (selectedQueueId) { setSelectedQueueId(""); setSelectedPatientId(""); } }} style={inputStyle} placeholder="Full name" />
              </div>
              <div style={{ marginBottom: 12 }}>
          ...
          {/* Footer */}
          <div style={{ padding: "14px 24px", borderTop: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, background: "#fafafa", flexShrink: 0 }}>
            <span style={{ fontSize: 13, color: "#6b7280" }}>
              {selectedCount > 0 ? `${selectedCount} test${selectedCount > 1 ? "s" : ""} selected` : "No tests selected yet"}
            </span>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={requestClose} style={{ padding: "9px 24px", borderRadius: 8, border: "1px solid #d1d5db", background: "#fff", fontSize: 14, cursor: "pointer", fontWeight: 600, color: "#374151" }}>CANCEL</button>
