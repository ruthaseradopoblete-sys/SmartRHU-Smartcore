...
export default function Sidebar({ active, setActive }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { label:'Medicine Stock', icon: Users,           section:'Menu',    badge: null },
  ...
  const NavBtn = ({ label, icon: Icon, badge }: {
    label: string; icon: React.ElementType; badge: string | null
  }) => {
    const isActive = active === label.toLowerCase().replace(' ', '')
      || (label === 'Dashboard'      && active === 'dashboard')
      || (label === 'Medicine Stock' && active === 'stock')
    ...
    const handleClick = () => {
      ...
      else if (label === 'Medicine Stock') setActive('stock')
      else if (label === 'Settings')  setActive('settings')
    }

    return (
      <button
        className={`srhu-nav-btn${isActive ? ' active-btn' : ''}`}
        onClick={handleClick}
        ...
        style={{
          ...
          background: isActive
            ...
            : hovered
              ...
              : 'transparent',
          color: isActive ? '#fff' : darkMode ? '#7bc98a' : '#3a6340',
      ...
      <aside className="srhu-sidebar" style={{
        ...
        <nav style={{ padding:'14px 10px 0', flex:1, position:'relative', zIndex:1 }}>
          ...
          {menuItems.filter(i => i.section === 'Menu').map(({ label, icon, badge }) => (
            <NavBtn key={label} label={label} icon={icon} badge={badge} />
          ))}
          ...
          <div>
            ...
            <button
              ...
              onMouseEnter={() => setHoveredItem('_logout_')}
              onMouseLeave={() => setHoveredItem(null)}
              style={{
                width:'100%', display:'flex', alignItems:'center',
                gap: 10, justifyContent: 'flex-start',
                ...
                background: hoveredItem === '_logout_'
                  ? 'rgba(229,62,62,0.07)'
                  : 'transparent',
            ...
            >
              <span style={{
                ...
                background: hoveredItem === '_logout_' ? 'rgba(229,62,62,0.1)' : 'transparent',
                transition:'background 0.18s',
                flexShrink:0,
              }}>
