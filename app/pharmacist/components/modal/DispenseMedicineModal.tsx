...
const IS_BOX_UNIT = (unit: string) =>
  unit?.trim().toLowerCase().includes("box");
...
async function exportToPDF(rows: Medicine[], tabLabel: string, isDrugs: boolean) {
  ...
  autoTable(doc, {
    startY: 27,
    head: [["#", "Medicine", isDrugs ? "Dosage" : "Specification", "Type", "Unit", "EXP Date", "Stock", "Boxes", "Partial", "Pcs/Box"]],
    body: rows.map((m, i) => {
      const isBox = IS_BOX_UNIT(m.unit);
      const ppb   = isBox ? (m.pieces_per_box ?? 0) : 0;
      const total = isBox && ppb > 0
        ? (m.boxes ?? 0) * ppb + (m.partial_pieces ?? 0)
        : m.quantity;
      return [
        i + 1, m.med_name, m.med_dosage, m.med_type, m.unit, m.exp_date,
        total,
        isBox ? (m.boxes ?? 0) : "—",
        isBox ? (m.partial_pieces ?? 0) : "—",
        isBox && ppb > 0 ? ppb : "—",
      ];
    }),
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  const isBox      = IS_BOX_UNIT(med.unit);
                  const piecesPerBox = isBox ? (med.pieces_per_box ?? 0) : 0;
                  const effectiveQty = isBox && piecesPerBox > 0
