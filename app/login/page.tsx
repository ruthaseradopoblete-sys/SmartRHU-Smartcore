...
import { logAction } from "@/utils/auditLog";   // ← DAGDAG
...
export default function LoginPage() {
  ...
  async function handleSignIn(e: React.FormEvent, forAdmin = false) {
    ...
    try {
      ...
      if (!authErr) {
        ...
        const { error: a2 } = await supabase.auth.signInWithPassword({ email: found.email, password });
  ...
  async function handleChangePassword(e: React.FormEvent) {
    ...
    try {
      if (!pwConditions.every(c => c.met)) throw new Error("Password does not meet all conditions.");
      ...
      const { error: dbErr } = await supabase
        .from("users").update({ is_first_login: false }).eq("user_id", userId);
      ...
      const { error: reAuthErr } = await supabase.auth.signInWithPassword({ email: userEmail, password: newPw });
      if (reAuthErr) throw new Error("Re-login failed: " + reAuthErr.message);

      const { data: freshProfile } = await supabase
        .from("users").select("is_first_login").eq("user_id", userId).single();
      ...
      <div className={styles.formPanel}>
        <div className={styles.formTop}>
          ...
          <form className={styles.formInner} onSubmit={handleChangePassword}>
            ...
            {cpError && <div className={styles.errorBox}><span>⚠️</span> {cpError}</div>}
            ...
            <p style={{ fontSize: 12, color: "#374151", marginBottom: 6, fontWeight: 600 }}>Conditions:</p>
