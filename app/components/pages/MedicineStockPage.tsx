...
export default function MedicineStockPage({ onToast }: Props) {
  ...
  const fetchMedicines = useCallback(async () => {
    setLoading(true);
    try {
      const data = await supabase("GET", "pharma_medicines",
        undefined, "archived=eq.false&order=created_at.desc");
  ...
  const archiveSelected = async () => {
    ...
    try {
      for (const id of selected) {
        await supabase("PATCH", `pharma_medicines?id=eq.${id}`, { archived: true });
      }
