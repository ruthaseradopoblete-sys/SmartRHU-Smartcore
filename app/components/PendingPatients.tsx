...
export default function PendingPatients({ onConsult }: Props) {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    const patientIds = consultRows
      .map((r: any) => r.patient_id)
      .filter(Boolean);
    const { data: patientRows } = await supabase
      .from("patients")
      .select(
        "id, first_name, last_name, age, sex, purok, barangay, municipality",
      )
      .in("id", patientIds);
    ...
    const entries: QueueEntry[] = consultRows
      .map((row: any, index: number) => {
        ...
        return {
          ...
          civil: "",
          addr: [p.purok, p.barangay, p.municipality]
            .filter(Boolean)
            .join(", "),
  ...
  // ── Completed Consultations — SHOW ALL DONE CONSULTATIONS ───────────────
  // Dito lalabas LAHAT ng completed/done SOAP consultations, kahit archived na,
  // at hindi na dini-deduplicate by patient. Kung 1 patient ay may 3 completed
  // consultations, 3 records din ang lalabas at bawat isa ay pwedeng i-view.
  const fetchCompleted = useCallback(async () => {
    ...
    try {
      const consultRows = await fetchAll((from, to) =>
        supabase
          .from("soap_consultations")
          .select(
            "id, patient_id, consultation_date, queue_date, created_at, status",
          )
          .in("status", ["done", "completed"])
          .order("consultation_date", { ascending: false, nullsFirst: false })
          .order("queue_date", { ascending: false, nullsFirst: false })
      ...
      // Fetch all patients by pages para hindi magkaproblema sa sobrang habang .in(...)
      const patientRows = await fetchAll((from, to) =>
        supabase
          .from("patients")
          .select(
            "id, first_name, last_name, age, sex, purok, barangay, municipality",
      ...
      const entries: CompletedConsult[] = consultRows
        .map((row: any) => {
          const p = patientMap[row.patient_id];
          if (!p) return null;

          const displayDate =
            row.consultation_date ?? row.queue_date ?? row.created_at ?? null;
  ...
  const filteredCompleted = completed.filter((c) => {
    ...
    if (!q) return true;
    const dateText = c.consultationDate
      ? new Date(c.consultationDate)
        ...
        {(
          ...
          <button
            ...
            style={{
              ...
              borderBottom:
                tab === t ? "2px solid var(--green)" : "2px solid transparent",
              transition: "all .15s",
            }}
          >
            {label}
      ...
      {tab === "queue" && (
        <div className={styles.pendingList}>
          ...
          {!loading &&
            ...
            (() => {
              const firstDoneIndex = queue.findIndex(
                (q) => q.status === "done",
              );
      ...
      {tab === "completed" && (
        <div className={styles.pendingList}>
          ...
          {!loadingCompleted && filteredCompleted.length === 0 && (
            <div className={styles.emptyState}>
              {search
                ? `No completed consultations found for "${search}"`
          ...
          {!loadingCompleted &&
            filteredCompleted.map((c) => (
              <div key={c.consultId} className={styles.pendingItem}>
  ...
  return (
    ...
    >
      <div className={styles.pendingItemTop}>
        ...
        <span
          className={`${styles.statusPill} ${p.status === "done" ? styles.statusDone : styles.statusWaiting}`}
        >
...
function SoapViewModal({
    ...
    async function load() {
      ...
      const [cR, vR] = await Promise.all([
        supabase
          .from("soap_consultations")
          .select(
            "consultation_date, subjective, objective, plan, assessments, icd10_codes, follow_up_date, follow_up_notes",
          )
          .eq("id", consultId)
          .maybeSingle(),
        supabase
          ...
          .select(
            "blood_pressure_mmhg, heart_rate_bpm, temperature_c, respiratory_rate_cpm, weight_kg, height_cm",
          )
  ...
  const hasVitals =
    vitals &&
    (vitals.blood_pressure_mmhg ||
      vitals.heart_rate_bpm ||
      vitals.temperature_c ||
      vitals.respiratory_rate_cpm ||
      vitals.weight_kg ||
      vitals.height_cm);

  const assessmentList =
    data?.assessments && data.assessments.length ? data.assessments : null;
          ...
          <div>
            ...
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.8)" }}>
              ...
              {data?.consultation_date
                ? new Date(data.consultation_date).toLocaleDateString("en-PH", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })
        ...
        >
          ...
          {!loading && data && (
            <>
                ...
                <div style={{ marginBottom: 18 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    <VitalPill
                      icon="❤️"
                      label="BP"
                      value={vitals.blood_pressure_mmhg}
                      unit="mmHg"
                    />
                    <VitalPill
                      icon="💗"
                    ...
                    <VitalPill
                      icon="🫁"
                      label="RR"
                      value={vitals.respiratory_rate_cpm}
                      unit="cpm"
                    />
                    <VitalPill
                      icon="⚖️"
              ...
              >
                {SOAP_CFG.map(
                  ({ key, label, icon, bg, border, color, tag }) => (
                    <div
                      key={key}
                      style={{
                        background: bg,
              ...
              {data.follow_up_date && (
                ...
                >
                  ...
                  <div style={{ fontSize: 13, color: "#78350f" }}>
                    {new Date(data.follow_up_date).toLocaleDateString("en-PH", {
                      month: "long",
                      day: "numeric",
                      year: "numeric",
                    })}
