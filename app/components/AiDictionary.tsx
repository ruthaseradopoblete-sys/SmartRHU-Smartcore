...
function detectLanguage(text: string): "tagalog" | "english" {
  const tagalogWords = [
    "para",
    "paano",
    "ano",
  ...
  const tagalogScore = tagalogWords.filter((word) =>
    lower.includes(` ${word} `)
  ).length;
...
function MedicineCard({ med }: { med: Medicine }) {
  ...
  return (
    <div className={styles.medCard}>
        ...
        <span
          className={`${styles.medDetail} ${
            isExpired
              ? styles.medExpired
              : isExpiringSoon
              ? styles.medExpiringSoon
              : ""
          }`}
        >
          Exp:{" "}
          {expDate.toLocaleDateString("en-PH", {
            month: "short",
            year: "numeric",
          })}
      ...
      {med.description && <p className={styles.medDesc}>{med.description}</p>}
    </div>
  );
}
...
export default function AiDictionary() {
  ...
  const [log, setLog] = useState<ChatMessage[]>([
    {
      from: "ai",
      text:
        "Hello, Doctor! I'm your AI Medical Dictionary powered by SmartRHU's live medicine inventory. Describe your patient's symptoms, age, and condition — I'll search our database and suggest compatible medicines.",
    },
  ]);
  ...
  async function send(query?: string) {
    ...
    const language = detectLanguage(m);
    ...
    try {
      ...
      setLog((l) => {
        const filtered = l.filter((message) => !message.loading);
            ...
            {m.loading ? (
              ...
              <div
                className={`${styles.chatBubble} ${
                  m.from === "ai" ? styles.chatAi : styles.chatUser
                }`}
              >
                {m.text}
              </div>
            ...
            {m.medicines && m.medicines.length > 0 && (
              <div className={styles.medicineSection}>
                <p className={styles.medicineSectionLabel}>
                  {m.medicines.length} medicine
                  {m.medicines.length > 1 ? "s" : ""} found in inventory
                </p>
            ...
            {m.medicines &&
              ...
              !m.loading && (
                <div className={styles.noMeds}>
                  No matching medicines in current inventory.
                </div>
              )}
          </div>
        ))}
