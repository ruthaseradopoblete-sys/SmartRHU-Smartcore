...
// ── PH-date helper ────────────────────────────────────────────────────────────
// Ginagawang "YYYY-MM-DD" sa Asia/Manila timezone. ITO ang dapat gamitin para sa
// request_date — para tumugma sa getTodayPH() ng DoctorDashboard. Huwag gamitin
// ang new Date().toISOString() dahil UTC iyon at mag-iiba ng petsa tuwing madaling-araw.
...
export default function LabRequestModal({ open, patient, onClose, onSend, doctorName }: Props) {
  ...
  // ✅ FIX: gumamit ng PH date, hindi UTC. Dati: new Date().toISOString().split("T")[0]
  ...
  // Prevent background/page scrollbar and white side gaps while the modal is open
  useEffect(() => {
    if (!open) return;
  ...
  useEffect(() => {
    ...
    // ✅ FIX: PH date din dito (dati: new Date().toISOString().split("T")[0])
  ...
  async function handleConfirmedSend() {
    ...
    const { error: dbError } = await supabase
      ...
      .insert({
        ...
        request_date: date,                  // ✅ PH date na (tingnan getTodayPHDate)
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{
          background: "#fff", borderRadius: 16, width: "min(980px, calc(100vw - 32px))", maxWidth: "100%",
          height: "min(92vh, 820px)", maxHeight: "92vh", display: "flex", flexDirection: "column",
