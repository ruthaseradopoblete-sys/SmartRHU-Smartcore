...
/* ── Types ── */
type TestType = 'Fecalysis' | 'Urinalysis' | 'Hematology' | 'Clinical Chemistry' | 'Serology'
type SortMode = 'none' | 'az' | 'asc' | 'desc'
type MenuLabel = 'Dashboard' | 'Patient Laboratory Records' | 'Settings' | 'Help'
...
interface SerologyData {
  [key: string]: string | SerologyRowData
  remarks?: string
}
...
/* ── Sidebar ── */
const MENU_ITEMS: { label: MenuLabel; section: string }[] = [
...
interface SidebarProps {
  ...
  setActiveMenu: (menu: MenuLabel) => void
  sidebarOpen: boolean
  onLogout: () => void
  darkMode: boolean
}

function LabSidebar({ activeMenu, setActiveMenu, sidebarOpen, onLogout, darkMode }: SidebarProps) {
  const bg = darkMode ? '#0a1a0d' : '#ffffff'
  const bdr = darkMode ? '1px solid #1a3320' : '1px solid #dceadc'

  const NavBtn = ({ label, active }: { label: MenuLabel; active: boolean }) => (
    <button
...
const F = ({ label, value, onChange, width = '100%', type = 'text', minLabelW = 100 }: FProps) => (
...
function FecalysisForm({ data, setData }: FormProps) {
  const S = (k: string) => (
    <input
      value={(data[k] as string) || ''}
      onChange={e => setData((p: LabData) => ({...p, [k]: e.target.value}))}
      style={{ width:'100%', border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }}
    />
  )
  return (
    <div style={{ border:'1px solid #ccc', borderRadius:4 }}>
      ...
      <div style={{ padding:'10px 14px' }}>
        ...
        ] as [string[], string[], string][]).map(([left, right, nv], idx) => (
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:6 }}>
          ...
          <input value={(data.parasite as string) || ''} onChange={e => setData((p: LabData) => ({...p, parasite: e.target.value}))} style={{ flex:1, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }} />
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:5 }}>
          <label style={{ fontSize:11, minWidth:60 }}>Others:</label>
          <input value={(data.others as string) || ''} onChange={e => setData((p: LabData) => ({...p, others: e.target.value}))} style={{ width:220, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }} />
        </div>
      </div>
    </div>
  )
...
function UrinalysisForm({ data, setData }: FormProps) {
  const rows: ([string, string] | null)[][] = [
    ...
    [['Protein','protein'],'Negative' as unknown as [string,string],['Mucus Thread','mucus'],null],
    [['Sugar','sugar'],'Negative' as unknown as [string,string],['Bacteria','bacteria'],null],
  ]
  const S = (k: string) => (
    <input
      value={(data[k] as string) || ''}
      onChange={e => setData((p: LabData) => ({...p, [k]: e.target.value}))}
      style={{ flex:1, border:'1px solid #bbb', borderRadius:3, padding:'2px 4px', fontSize:11, outline:'none', minWidth:0 }}
    />
  )
  return (
    <div style={{ border:'1px solid #ccc', borderRadius:4 }}>
      ...
      <div style={{ padding:'10px 14px' }}>
        ...
        {rows.map((row, idx) => {
          const l = row[0] as [string, string] | null
          const ln = row[1] as string | null
          const r = row[2] as [string, string] | null
          const rn = row[3] as string | null
          return (
            <div key={idx} style={{ display:'grid', gridTemplateColumns:'1fr 70px 1fr 70px', gap:4, alignItems:'center', marginBottom:3 }}>
              <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                {l && <><label style={{ fontSize:11, minWidth:90, flexShrink:0 }}>{l[0]}:</label>{S(l[1])}</>}
              </div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:6 }}>
          ...
          <input value={(data.others as string) || ''} onChange={e => setData((p: LabData) => ({...p, others: e.target.value}))} style={{ flex:1, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }} />
        </div>
      </div>
    </div>
  )
...
function HematologyForm({ data, setData }: FormProps) {
  const S = (k: string, w: string = '70px') => (
    <input
      value={(data[k] as string) || ''}
      onChange={e => setData((p: LabData) => ({...p, [k]: e.target.value}))}
      style={{ width:w, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }}
    />
  )
  ...
  const diff: [string, string, string][] = [
    ...
    ['Monocytes','mono','2–8%'],['Eosinophils','eos','1–4%'],['Basophils','baso','0–1%']
  ]
  return (
    <div style={{ border:'1px solid #ccc', borderRadius:4 }}>
      ...
      <div style={{ padding:'10px 14px' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          ...
          <input value={(data.remarks as string) || ''} onChange={e => setData((p: LabData) => ({...p, remarks: e.target.value}))} style={{ flex:1, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }} />
        </div>
      </div>
    </div>
  )
...
function ClinicalChemForm({ data, setData }: FormProps) {
  const S = (k: string, w: string = '70px') => (
    <input
      value={(data[k] as string) || ''}
      onChange={e => setData((p: LabData) => ({...p, [k]: e.target.value}))}
    ...
    <div style={{ border:'1px solid #ccc', borderRadius:4 }}>
      ...
      <div style={{ padding:'10px 14px' }}>
        ...
        <div style={{ borderTop:'1px solid #ddd', paddingTop:8, display:'flex', flexDirection:'column', gap:5 }}>
          {([['Remarks','remarks'],['Last Meal','lastMeal'],['Time of Extraction','timeEx']] as [string,string][]).map(([lbl,k]) => (
            <div key={k} style={{ display:'flex', alignItems:'center', gap:6 }}>
              <label style={{ fontSize:11, minWidth:130 }}>{lbl}:</label>
              <input value={(data[k] as string) || ''} onChange={e => setData((p: LabData) => ({...p, [k]: e.target.value}))} style={{ flex:1, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }} />
            </div>
          ))}
        </div>
      </div>
...
interface SerologyFormProps {
  data: SerologyData
  setData: React.Dispatch<React.SetStateAction<SerologyData>>
}

function SerologyForm({ data, setData }: SerologyFormProps) {
    ...
    <div style={{ border:'1px solid #ccc', borderRadius:4 }}>
      ...
      <div style={{ padding:'10px 14px' }}>
          ...
          <tbody>
            {rows.map(([lbl, key]) => (
              <tr key={key}>
                ...
                {(['kit','lot','exp','type','result'] as (keyof SerologyRowData)[]).map(f => (
                  <td key={f} style={{ padding:'2px 4px', border:'1px solid #ccc' }}>
                    <input
                      value={((data[key] as SerologyRowData) || {})[f] || ''}
                      onChange={e => setData((p: SerologyData) => ({ ...p, [key]: { ...((p[key] as SerologyRowData) || {}), [f]: e.target.value } }))}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:8 }}>
          <label style={{ fontSize:11, minWidth:60 }}>Remarks:</label>
          <input value={(data.remarks as string) || ''} onChange={e => setData((p: SerologyData) => ({...p, remarks: e.target.value}))} style={{ flex:1, border:'1px solid #bbb', borderRadius:3, padding:'2px 5px', fontSize:11, outline:'none' }} />
        </div>
      </div>
    </div>
  )
...
/* ── Lab Form Modal ── */
const TESTS: TestType[] = ['Fecalysis','Urinalysis','Hematology','Clinical Chemistry','Serology']
...
function LabFormModal({ isOpen, onClose, patient, initialTest }: LabFormModalProps) {
  const [selTest, setSelTest] = useState<TestType>(initialTest || 'Fecalysis')
  ...
  const [fec, setFec] = useState<LabData>({})
  const [uri, setUri] = useState<LabData>({})
  const [hem, setHem] = useState<LabData>({})
  const [chem, setChem] = useState<LabData>({})
  const [ser, setSer] = useState<SerologyData>({})
  ...
  const LabHeader = () => (
    <div style={{ border:'2px solid #1a6b2e', borderRadius:4, marginBottom:12 }}>
      ...
      <div style={{ padding:'10px 12px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4px 18px' }}>
        <F label="Name" value={labInfo.name} onChange={(v: string) => setLabInfo(p => ({...p, name:v}))} minLabelW={55}/>
        <F label="Date" value={labInfo.date} onChange={(v: string) => setLabInfo(p => ({...p, date:v}))} type="date" width="130px" minLabelW={40}/>
        <F label="Address" value={labInfo.address} onChange={(v: string) => setLabInfo(p => ({...p, address:v}))} minLabelW={55}/>
        <F label="Age" value={labInfo.age} onChange={(v: string) => setLabInfo(p => ({...p, age:v}))} width="60px" minLabelW={40}/>
        <F label="Req. Physician" value={labInfo.reqPhys} onChange={(v: string) => setLabInfo(p => ({...p, reqPhys:v}))} minLabelW={95}/>
        <F label="Sex" value={labInfo.sex} onChange={(v: string) => setLabInfo(p => ({...p, sex:v}))} width="80px" minLabelW={40}/>
      </div>
    </div>
  )
  ...
  const overlayStyle: CSSProperties = {
    ...
    justifyContent:'center', alignItems:'flex-start', zIndex:1000, padding:16, overflowY:'auto'
  }

  return (
    <div style={overlayStyle}>
      <div style={{ background:'#fff', width:'100%', maxWidth:880, borderRadius:8, fontFamily:'Arial,sans-serif', marginTop:8, marginBottom:8 }}>
        {/* Header */}
        <div style={{ background:'#1a6b2e', color:'#fff', padding:'9px 16px', fontWeight:700, fontSize:14, borderRadius:'8px 8px 0 0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', borderBottom:'2px solid #1a6b2e' }}>
          <div style={{ padding:'12px 16px', borderRight:'1px solid #ddd' }}>
            ...
            <F label="Name" value={pInfo.name} onChange={(v: string) => setPInfo(p => ({...p, name:v}))} minLabelW={55}/>
            ...
            <F label="Address" value={pInfo.address} onChange={(v: string) => setPInfo(p => ({...p, address:v}))} minLabelW={55}/>
...
interface AddPatientModalProps {
  isOpen: boolean
  onClose: () => void
  onSaved: (form: AddPatientForm) => void
}

function AddPatientModal({ isOpen, onClose, onSaved }: AddPatientModalProps) {
  const [form, setForm] = useState<AddPatientForm>({ name:'', age:'', gender:'', address:'', contact:'', email:'', test:'Fecalysis' })
  ...
  const inp = (label: string, key: keyof AddPatientForm, type: string = 'text') => (
    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:7 }}>
      ...
      <input
        ...
        style={{ flex:1, border:'1px solid #aaa', borderRadius:3, padding:'4px 8px', fontSize:13, outline:'none' }}
      />
    </div>
  )
...
interface DashboardProps {
  patients: Patient[]
  onAddTest: (p: Patient) => void
  onAddPatient: () => void
  darkMode: boolean
}

function LabDashboard({ patients, onAddTest, onAddPatient, darkMode }: DashboardProps) {
  const [labFilter, setLabFilter] = useState<TestType>('Serology')
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:18 }}>
        <div style={{ background:cardBg, borderRadius:16, padding:18, boxShadow:'0 2px 8px rgba(0,0,0,.07)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
            ...
            <select value={labFilter} onChange={e => setLabFilter(e.target.value as TestType)} style={{ border:'1px solid #ddd', borderRadius:5, padding:'3px 8px', fontSize:11, outline:'none', background: darkMode?'#0d2010':'#fff', color: darkMode?'#7ab88a':'#333' }}>
...
/* ── Patient Lab Records ── */
interface PatientLabRecordsProps {
  patients: Patient[]
  onAddPatient: () => void
...
function PatientLabRecords({ patients, onAddPatient, onViewPatient, darkMode }: PatientLabRecordsProps) {
  ...
  if (sortMode === 'asc')  display = [...display].sort((a,b) => new Date(a.created).getTime() - new Date(b.created).getTime())
  if (sortMode === 'desc') display = [...display].sort((a,b) => new Date(b.created).getTime() - new Date(a.created).getTime())
  ...
  const TBBtn = ({ label, active, fn }: { label: string; active: boolean; fn: () => void }) => (
...
export default function LaboratoryPage() {
  ...
  const [labForm, setLabForm] = useState<{ open: boolean; patient: Patient | null; test: TestType }>({ open:false, patient:null, test:'Fecalysis' })
  const [showNotif, setShowNotif] = useState(false)

  const openLab = (p: Patient | null, t?: TestType) => setLabForm({ open:true, patient: p||null, test: t || 'Fecalysis' })

  const addPatient = (f: AddPatientForm) => {
    setPatients(p => [...p, {
      id: _nextId++,
