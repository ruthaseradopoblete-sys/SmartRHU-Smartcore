...
function CB({ checked }) {
  return (
    <div style={{
      width: 8, height: 8, border: '1px solid #000', borderRadius: 1,
      flexShrink: 0, display: 'inline-flex', alignItems: 'center',
      justifyContent: 'center', background: checked ? '#000' : '#fff',
    }}>
...
function SectionTitle({ children }) {
  return (
    <div style={{
      ...
      letterSpacing: 0.2, marginBottom: 2, marginTop: 4,
    }}>
      {children}
    </div>
  )
    ...
    <div className="lrf-form" style={{
      ...
      <div style={{ display: 'flex', gap: 0 }}>
        ...
        <div style={{ flex: 1, fontSize: 9, paddingRight: 5, borderRight: B }}>
          ...
          <Row label="PT, PTT"                 checked={tests.pt_ptt} />
          ...
          <Row label="BUN"                 checked={tests.bun} />
          <Row label="Creatinine"          checked={tests.creatinine} />
          <Row label="SGPT (ALT)"          checked={tests.sgpt_alt} />
          <Row label="SGOT (AST)"          checked={tests.sgot_ast} />
          <Row label="Serum Na, K, Cl"     checked={tests.serum_na_k_cl} />
        ...
        <div style={{ flex: 1, paddingLeft: 5 }}>
          ...
          <Row label="Typhidot IgG/IgM"     checked={tests.typhidot_igg_igm} />
          <Row label="HbsAg"                checked={tests.hbsag} />
          <Row label="12 Lead ECG"          checked={tests.ecg_12_lead} />
    ...
    <>
      ...
      <div className="lrf-no-print" style={{
        ...
        <div style={{
          ...
          {/* Form preview */}
          <div style={{
            overflowY: 'auto', flex: 1, display: 'flex', justifyContent: 'center',
            padding: '20px 0', background: '#f3f4f6', opacity: 1, filter: 'none',
          }}>
            <FormBody
              name={name} age={age} gender={gender} civil={civil}
              address={address} date={date} tests={tests}
              ultrasound={patient.ultrasound || ''}
              xray={patient.xray || ''}
              othersText={patient.others || ''}
            />
          </div>
        </div>
      </div>
