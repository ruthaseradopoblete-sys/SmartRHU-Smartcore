...
const injectStyles = () => {
  ...
  style.textContent = `
    ...
    .srhu-admin-nav-btn::after {
      content: '';
      position: absolute;
      inset: 0;
      border-radius: 12px;
      background: linear-gradient(90deg, rgba(109,40,217,0.1) 0%, transparent 100%);
      opacity: 0;
      transition: opacity 0.2s ease;
    }
    .srhu-admin-nav-btn:hover::after { opacity: 1; }
    .srhu-admin-nav-btn.active-btn::after { display: none; }

    .srhu-admin-pulse {
      animation: admin-pulse-dot 2.5s ease-in-out infinite;
    }
    ...
    .srhu-admin-slide-in {
      animation: adminSlideIn 0.3s cubic-bezier(0.22, 1, 0.36, 1) both;
    }
    ...
    .srhu-admin-badge {
      ...
      font-weight: 700;
      padding: 0 5px;
      background: linear-gradient(135deg, #22c55e, #1a7a1a);
      color: #fff;
      margin-left: auto;
...
export default function AdminSidebar({ activeMenu, setActiveMenu, sidebarOpen, onLogout, darkMode }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { label:'Patient Records',      icon: Users,           section:'Menu',       badge: null  },
    { label:'Lab Records',          icon: FlaskConical,    section:'Menu',       badge: null  },
    { label:'Inventory Records',    icon: Package,         section:'Menu',       badge: null  },
    { label:'Generate Report',      icon: FileBarChart,    section:'Menu',       badge: null  },
    { label:'User Management',      icon: UserCog,         section:'Admin',      badge: null  },
    { label:'Roles & Permissions',  icon: ShieldCheck,     section:'Admin',      badge: null  },
    { label:'System Activities',    icon: Monitor,         section:'Admin',      badge: null  },
    { label:'Backup & Restore',     icon: Database,        section:'Admin',      badge: null  },
  ...
  const NavBtn = ({ label, icon: Icon, active, badge }: {
    ...
    return (
      <button
        className={`srhu-admin-nav-btn${active ? ' active-btn' : ''}`}
        ...
        style={{
          ...
          background: active
            ? darkMode
              ? 'linear-gradient(135deg, #0a1a0d 0%, #1a4d1a 100%)'
              : 'linear-gradient(135deg, #1a7a1a 0%, #26a326 100%)'
            : hovered
              ? darkMode ? 'rgba(26,122,26,0.08)' : 'rgba(26,122,26,0.07)'
              : 'transparent',
      ...
      >
        ...
        <span style={{
          ...
          background: active
            ...
            : hovered
              ? darkMode ? 'rgba(26,122,26,0.12)' : 'rgba(109,40,217,0.1)'
              : 'transparent',
        ...
        {sidebarOpen && (
          <>
            ...
            {badge && <span className="srhu-admin-badge">{badge}</span>}
          </>
        )}
      </button>
    )
    ...
    <aside className="srhu-admin-sidebar" style={{
      ...
      <div style={{
        ...
        background: darkMode
          ...
          : 'radial-gradient(circle, rgba(26,122,26,0.1) 0%, transparent 70%)',
        pointerEvents:'none',
      }} />
      <div style={{
        ...
        {sidebarOpen && (
          <div className="srhu-admin-slide-in" style={{ overflow:'hidden' }}>
            ...
            <div style={{
              ...
              color: darkMode ? '#3a6b48' : '#9f7ad8',
              marginTop:4, whiteSpace:'nowrap',
              display:'flex', alignItems:'center', gap:5,
            }}>
              <span className="srhu-admin-pulse" style={{
      ...
      <nav style={{ padding:'14px 10px 0', flex:1, position:'relative', zIndex:1, overflowY:'auto', overflowX:'hidden' }}>
        ...
        {/* Divider */}
        <div style={{ height:1, margin:'10px 6px', background:`linear-gradient(90deg, transparent, ${borderCol} 30%, ${borderCol} 70%, transparent)` }} />
        ...
        {menuItems.filter(i => i.section === 'Admin').map(({ label, icon, badge }) => (
          <NavBtn key={label} label={label} icon={icon} active={activeMenu === label} badge={badge} />
        ))}
        ...
        <button
          ...
          style={{
            ...
            justifyContent: sidebarOpen ? 'flex-start' : 'center',
            padding: sidebarOpen ? '9px 14px' : '9px',
            borderRadius:12, marginTop:2, marginBottom:16,
      ...
      {sidebarOpen && (
        <div style={{
          ...
          background: darkMode ? 'rgba(10,20,10,0.85)' : 'rgba(255,255,255,0.85)',
          borderRadius:14, border:`1px solid ${borderCol}`,
          backdropFilter:'blur(8px)',
          boxShadow: darkMode
            ? 'inset 0 1px 0 rgba(26,122,26,0.1)'
